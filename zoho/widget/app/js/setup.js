...
var Setup = (function () {
  ...
  function steps() {
    ...
    var admins = emps.filter(function (e) { return (e.Roles || []).indexOf('管理者') >= 0; });
    var me = App.me();
  ...
  function renderFirstRun(el) {
    ...
    el.innerHTML =
      ...
      '権限は「管理者」で登録されます。入力した部署名は、部署マスタにも同時に登録されます。' +
      '</div>' +
  ...
  function createFirstAdmin(el) {
    ...
    DB.add('Departments', {
      ...
      return DB.add('Employees', {
        ...
        Roles: ['申請者', '承認者', '管理者'], Join_Date: '', Paid_Leave_Balance: 0,
