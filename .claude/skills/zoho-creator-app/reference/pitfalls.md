...
## 12. SDK が読み込まれているのに「SDK未検出」で止まる

**原因**：SDK の有無を `typeof ZOHO.CREATOR.init === 'function'` で判定した。
v2 の SDK（`version/2.0/widgetsdk-min.js`）には `init()` が無い（v1 の関数）。
実際の Creator では、コンソールに `widgetsdk-min.js` からのメッセージが出ていて SDK 自体は動いていたのに、
ウィジェットは「Zoho Creator に接続できませんでした（SDK未検出）」で止まった。

このスキルの初版の雛形も同じ判定だった。初版の雛形は、SDK が見つからないとデモモードに落ちる。
そのため Creator の中なのに「デモモード（未接続）」で起動し、入力が localStorage にしか保存されない、
という形で現れる。エラーにならないぶん、気づきにくい。

**対処**：

- SDK の判定は `ZOHO.CREATOR.DATA.getRecords` が関数かどうかで行う
- `init()` は、あるときだけ呼ぶ
- `getInitParams()` は Promise として受け取る（v2 は Promise を返す）
- Creator の中（iframe の中）では、SDK が使えなくてもデモへ落とさずエラー画面で止め、SDK が持つ関数の一覧を表示する

`verify/sdk-mock-test.js` は、`init()` の無い v2 の形のモックでも接続できるか、
iframe の中で SDK が無いときにデモへ落ちないかを確かめる。

**教訓**：動いているコードから読み取った呼び方でも、SDK の版が違えば通用しない。
呼ぶ前に関数があるかを確かめ、無ければ「何が有るか」を画面に出しておくと、1往復で原因が分かる。
